...
function showGreeting() {
  ...
  const name = (typeof _crProfile !== "undefined" && _crProfile?.displayName)
    || (typeof _crProfile !== "undefined" && _crProfile?.username)
    || localStorage.getItem("cr_display_name")
    || "";
  ...
  el.innerHTML = `
    <span class="home-greeting-emoji">${emoji}</span>
    <span class="home-greeting-text">${text}${name ? `<span class="home-greeting-name">, ${name}</span>` : ""}</span>
  `;
...
window.addEventListener("load", () => {
  showGreeting();
  // Re-run once auth loads in case profile wasn't ready
  if (typeof onAuthReady === "function") {
    onAuthReady(() => {
      document.getElementById("homeGreeting")?.remove();
      showGreeting();
    });
  }
});
...
let _crProfile     = null; // { username, bio, avatarUrl, createdAt, followers, following }
let _crAuthReady   = false;
let _crAuthCbs     = [];   // callbacks waiting for auth to be ready

function onAuthReady(cb) {
  if (_crAuthReady) { cb(_crUser, _crProfile); return; }
  _crAuthCbs.push(cb);
}
...
async function _getFirebase() {
  if (_fbApp && _fbAuth && _fbDb) return { app: _fbApp, auth: _fbAuth, db: _fbDb };
  const { initializeApp, getApps } = await import("https://www.gstatic.com/firebasejs/10.12.0/firebase-app.js");
  ...
  const { getDatabase, ref, set, get, update, push, onValue, off, remove, increment }
  ...
  // Store helpers on window for use in sub-functions
  ...
  onAuthStateChanged(_fbAuth, async user => {
    ...
    _crAuthReady = true;
    _crAuthCbs.forEach(cb => cb(user, _crProfile));
    _crAuthCbs = [];
    _updateProfileUI();
  });
...
async function _loadProfile(uid) {
  try {
    ...
    const snap = await get(ref(_fbDb, "users/" + uid + "/profile"));
...
async function _createProfile(user) {
  const username = (user.displayName || user.email?.split("@")[0] || "user")
    .toLowerCase().replace(/[^a-z0-9_]/g, "").slice(0, 20) + "_" + Math.random().toString(36).slice(2,5);
  const profile = {
    uid: user.uid,
    username,
  ...
  await set(ref(_fbDb, "users/" + user.uid + "/profile"), profile);
  await set(ref(_fbDb, "usernames/" + username), user.uid);
  return profile;
}

// ── Sync local data to cloud on login ─────────────────────────────────────
async function _syncLocalToCloud() {
  if (!_crUser) return;
  const { ref, set, get } = window._fbHelpers;
  ...
  try {
    ...
    if (localHistory.length) {
      const cloudSnap = await get(ref(_fbDb, "users/" + uid + "/history"));
      const cloudHistory = cloudSnap.exists() ? Object.values(cloudSnap.val()) : [];
      // Merge — prefer most recent entry per item
      const merged = {};
      [...cloudHistory, ...localHistory].forEach(h => {
        const k = (h.type||"movie") + "_" + (h.tmdbId||h.id);
        if (!merged[k] || (h.addedAt||0) > (merged[k].addedAt||0)) merged[k] = h;
      });
      await set(ref(_fbDb, "users/" + uid + "/history"), merged);
    }

    // Sync watchlist
    const localWL = JSON.parse(localStorage.getItem("watchlist") || "[]");
    if (localWL.length) {
      const cloudSnap = await get(ref(_fbDb, "users/" + uid + "/watchlist"));
      const cloudWL = cloudSnap.exists() ? Object.values(cloudSnap.val()) : [];
      const mergedWL = {};
      [...cloudWL, ...localWL].forEach(w => { mergedWL[(w.type||"movie") + "_" + w.id] = w; });
      await set(ref(_fbDb, "users/" + uid + "/watchlist"), mergedWL);
    }
  } catch(e) {}
}

// ── Cloud sync for save operations ───────────────────────────────────────
async function _cloudSaveHistory() {
  if (!_crUser) return;
  try {
    const { ref, set } = window._fbHelpers;
    const obj = {};
    historyData.forEach(h => { obj[(h.type||"movie") + "_" + (h.tmdbId||h.id)] = h; });
    await set(ref(_fbDb, "users/" + _crUser.uid + "/history"), obj);
  } catch(e) {}
}

async function _cloudSaveWatchlist() {
  if (!_crUser) return;
  try {
    const { ref, set } = window._fbHelpers;
    const obj = {};
    watchlistData.forEach(w => { obj[(w.type||"movie") + "_" + w.id] = w; });
    await set(ref(_fbDb, "users/" + _crUser.uid + "/watchlist"), obj);
  } catch(e) {}
}

// Patch saveHistory + saveWatchlist to also sync to cloud
const _origSaveHistory2  = saveHistory;
const _origSaveWatchlist2 = saveWatchlist;
window.saveHistory = function() { _origSaveHistory2(); _cloudSaveHistory(); };
window.saveWatchlist = function() { _origSaveWatchlist2(); _cloudSaveWatchlist(); };

// Load from cloud if logged in, fall back to local
async function loadFromCloud() {
  ...
  try {
    ...
    const [hSnap, wSnap] = await Promise.all([
      get(ref(_fbDb, "users/" + uid + "/history")),
      get(ref(_fbDb, "users/" + uid + "/watchlist")),
    ]);

    if (hSnap.exists()) {
      historyData = Object.values(hSnap.val());
      localStorage.setItem("history", JSON.stringify(historyData));
    }
    if (wSnap.exists()) {
      watchlistData = Object.values(wSnap.val());
      localStorage.setItem("watchlist", JSON.stringify(watchlistData));
    }
  } catch(e) {}
}
...
function openAuthModal(defaultTab) {
  document.getElementById("crAuthModal")?.remove();
  const modal = document.createElement("div");
  modal.id = "crAuthModal";
  modal.style.cssText = "position:fixed;inset:0;z-index:9900;background:rgba(0,0,0,0.82);backdrop-filter:blur(12px);display:flex;align-items:center;justify-content:center;padding:20px;";
  modal.innerHTML = `
    <div style="background:linear-gradient(160deg,#160202,#0a0606);border:1px solid rgba(255,44,44,0.2);border-radius:20px;width:100%;max-width:400px;overflow:hidden;box-shadow:0 24px 60px rgba(0,0,0,0.7);">
      <!-- Tabs -->
      <div style="display:flex;border-bottom:1px solid rgba(255,255,255,0.07);">
        <button id="crTabSignin" style="flex:1;padding:14px;background:none;border:none;color:#fff;font-weight:800;font-size:14px;cursor:pointer;border-bottom:2px solid #ff2c2c;">Sign In</button>
        <button id="crTabSignup" style="flex:1;padding:14px;background:none;border:none;color:rgba(255,255,255,0.4);font-weight:700;font-size:14px;cursor:pointer;border-bottom:2px solid transparent;">Sign Up</button>
      </div>

      <div style="padding:24px;">
        <!-- Google button -->
        <button id="crGoogleBtn" style="width:100%;padding:12px;background:rgba(255,255,255,0.07);border:1.5px solid rgba(255,255,255,0.15);border-radius:10px;color:#fff;font-weight:700;font-size:14px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:16px;transition:background 0.15s;">
        ...
        <div id="crAuthForm">
          <input id="crAuthEmail" type="email" placeholder="Email" autocomplete="email"
            style="width:100%;padding:11px 14px;border-radius:10px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.06);color:#fff;font-size:14px;outline:none;margin-bottom:10px;box-sizing:border-box;">
          <input id="crAuthPassword" type="password" placeholder="Password" autocomplete="current-password"
            style="width:100%;padding:11px 14px;border-radius:10px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.06);color:#fff;font-size:14px;outline:none;margin-bottom:4px;box-sizing:border-box;">
          <div style="text-align:right;margin-bottom:14px;">
            <button id="crForgotBtn" style="background:none;border:none;color:rgba(255,100,100,0.6);font-size:12px;cursor:pointer;">Forgot password?</button>
          </div>
          <button id="crAuthSubmit" style="width:100%;padding:12px;background:#ff2c2c;border:none;border-radius:10px;color:#fff;font-weight:800;font-size:14px;cursor:pointer;">Sign In</button>
          <div id="crAuthError" style="margin-top:10px;font-size:12px;color:#ff6b6b;text-align:center;min-height:18px;"></div>
        </div>

        <button id="crAuthClose" style="width:100%;margin-top:14px;padding:10px;background:none;border:1px solid rgba(255,255,255,0.08);border-radius:10px;color:rgba(255,255,255,0.4);font-size:13px;cursor:pointer;">Cancel</button>
      </div>
    </div>
  `;
  document.body.appendChild(modal);

  let isSignUp = defaultTab === "signup";
  const updateTab = () => {
    document.getElementById("crTabSignin").style.color = isSignUp ? "rgba(255,255,255,0.4)" : "#fff";
    document.getElementById("crTabSignup").style.color = isSignUp ? "#fff" : "rgba(255,255,255,0.4)";
    document.getElementById("crTabSignin").style.borderBottomColor = isSignUp ? "transparent" : "#ff2c2c";
    document.getElementById("crTabSignup").style.borderBottomColor = isSignUp ? "#ff2c2c" : "transparent";
    document.getElementById("crAuthSubmit").textContent = isSignUp ? "Create Account" : "Sign In";
    document.getElementById("crAuthPassword").autocomplete = isSignUp ? "new-password" : "current-password";
    document.getElementById("crForgotBtn").style.display = isSignUp ? "none" : "inline";
  };
  updateTab();

  document.getElementById("crTabSignin").onclick = () => { isSignUp = false; updateTab(); };
  document.getElementById("crTabSignup").onclick = () => { isSignUp = true; updateTab(); };
  document.getElementById("crAuthClose").onclick = () => modal.remove();
  modal.onclick = e => { if (e.target === modal) modal.remove(); };

  // Google sign in
  document.getElementById("crGoogleBtn").onclick = async () => {
    try {
      const { auth } = await _getFirebase();
      const { signInWithPopup, GoogleAuthProvider } = window._fbHelpers;
      await signInWithPopup(auth, new GoogleAuthProvider());
      modal.remove();
      showToast("Welcome to CineRealm! 🎬", "success");
    } catch(e) {
      document.getElementById("crAuthError").textContent = e.message?.replace("Firebase: ", "") || "Sign in failed";
    }
  };

  // Email/password
  document.getElementById("crAuthSubmit").onclick = async () => {
    const email = document.getElementById("crAuthEmail").value.trim();
    const pass  = document.getElementById("crAuthPassword").value;
    const errEl = document.getElementById("crAuthError");
    if (!email || !pass) { errEl.textContent = "Please fill in all fields"; return; }
    try {
      const { auth } = await _getFirebase();
      const { signInWithEmailAndPassword, createUserWithEmailAndPassword } = window._fbHelpers;
      if (isSignUp) {
      ...
      const msg = e.code === "auth/wrong-password" ? "Incorrect password"
        ...
        : e.message?.replace("Firebase: ","") || "Something went wrong";
      errEl.textContent = msg;
    }
  };

  // Forgot password
  document.getElementById("crForgotBtn").onclick = async () => {
    const email = document.getElementById("crAuthEmail").value.trim();
    if (!email) { document.getElementById("crAuthError").textContent = "Enter your email first"; return; }
    try {
      const { auth } = await _getFirebase();
      await window._fbHelpers.sendPasswordResetEmail(auth, email);
      document.getElementById("crAuthError").style.color = "#22c55e";
      document.getElementById("crAuthError").textContent = "Reset email sent!";
    } catch(e) { document.getElementById("crAuthError").textContent = "Failed to send reset email"; }
  };

  // Enter key
  [document.getElementById("crAuthEmail"), document.getElementById("crAuthPassword")].forEach(el => {
    el.addEventListener("keydown", e => { if (e.key === "Enter") document.getElementById("crAuthSubmit").click(); });
    el.onfocus = () => el.style.borderColor = "rgba(255,44,44,0.4)";
...
async function signOutUser() {
  try {
    const { auth } = await _getFirebase();
    await window._fbHelpers.signOut(auth);
...
function _updateProfileUI() {
  ...
  document.getElementById("crProfileBtn")?.remove();
  ...
  btn.id = "crProfileBtn";
  btn.style.cssText = "flex-shrink:0;cursor:pointer;position:relative;";

  if (_crUser) {
    const initial = (_crProfile?.displayName || _crProfile?.username || _crUser.email || "?")[0].toUpperCase();
    const avatar  = _crProfile?.avatarUrl || _crUser.photoURL || "";
    btn.innerHTML = `
      <div class="cr-avatar" title="${_crProfile?.displayName || _crProfile?.username || ''}">
        ${avatar
          ? `<img src="${avatar}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`
          : `<span style="font-size:15px;font-weight:800;color:#fff;">${initial}</span>`
        }
      </div>
    `;
    btn.onclick = () => openProfileDropdown(btn);
  } else {
    btn.innerHTML = `
      <button class="cr-signin-btn" onclick="openAuthModal('signin')">Sign In</button>
    `;
  }
...
function openProfileDropdown(anchor) {
  document.getElementById("crProfileDropdown")?.remove();
  const dd = document.createElement("div");
  dd.id = "crProfileDropdown";
  dd.className = "cr-profile-dropdown";

  const name     = _crProfile?.displayName || _crProfile?.username || _crUser?.email || "User";
  const username = _crProfile?.username || "";
  const avatar   = _crProfile?.avatarUrl || _crUser?.photoURL || "";
  const initial  = name[0].toUpperCase();

  dd.innerHTML = `
    <div class="cr-dd-header">
      <div class="cr-avatar" style="width:42px;height:42px;font-size:18px;">
        ${avatar ? `<img src="${avatar}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">` : `<span style="font-weight:800;color:#fff;">${initial}</span>`}
      </div>
  ...
  setTimeout(() => {
    document.addEventListener("click", function _close(e) {
      if (!dd.contains(e.target) && e.target !== anchor) {
        dd.remove(); document.removeEventListener("click", _close);
      }
    });
  }, 10);
}
...
function openEditProfileModal() {
  ...
  const p = _crProfile || {};
  const avatar = p.avatarUrl || _crUser?.photoURL || "";
  const initial = (p.displayName || p.username || "?")[0].toUpperCase();

  modal.innerHTML = `
    <div style="background:linear-gradient(160deg,#160202,#0a0606);border:1px solid rgba(255,44,44,0.2);border-radius:20px;width:100%;max-width:400px;box-shadow:0 24px 60px rgba(0,0,0,0.7);overflow:hidden;">
      <div style="padding:18px 22px;border-bottom:1px solid rgba(255,255,255,0.07);display:flex;align-items:center;justify-content:space-between;">
        <span style="font-size:16px;font-weight:900;color:#fff;">Edit Profile</span>
        <button id="crEditClose" style="background:none;border:none;color:rgba(255,255,255,0.4);font-size:20px;cursor:pointer;">✕</button>
      </div>
      <div style="padding:22px;">
        <!-- Avatar -->
        <div style="text-align:center;margin-bottom:20px;">
          <div class="cr-avatar" style="width:72px;height:72px;font-size:28px;margin:0 auto 10px;cursor:pointer;" id="crAvatarPreview" title="Click to change avatar URL">
            ${avatar ? `<img id="crAvatarImg" src="${avatar}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">` : `<span style="font-weight:800;color:#fff;">${initial}</span>`}
          </div>
          <input id="crAvatarInput" type="url" placeholder="Avatar image URL (optional)" value="${avatar}"
            style="width:100%;padding:9px 12px;border-radius:8px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.05);color:#fff;font-size:12px;outline:none;box-sizing:border-box;text-align:center;">
        </div>

        <div style="margin-bottom:12px;">
          <label style="font-size:11px;font-weight:700;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:0.5px;display:block;margin-bottom:6px;">Display Name</label>
          <input id="crEditName" type="text" value="${p.displayName || ''}" maxlength="30" placeholder="Your name"
            style="width:100%;padding:10px 12px;border-radius:10px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.06);color:#fff;font-size:14px;outline:none;box-sizing:border-box;">
        </div>

        <div style="margin-bottom:12px;">
          <label style="font-size:11px;font-weight:700;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:0.5px;display:block;margin-bottom:6px;">Username</label>
          <div style="display:flex;align-items:center;gap:0;">
            <span style="padding:10px 10px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-right:none;border-radius:10px 0 0 10px;color:rgba(255,255,255,0.4);font-size:14px;">@</span>
            <input id="crEditUsername" type="text" value="${p.username || ''}" maxlength="20" placeholder="username"
              style="flex:1;padding:10px 12px;border-radius:0 10px 10px 0;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.06);color:#fff;font-size:14px;outline:none;box-sizing:border-box;">
          </div>
          <div id="crUsernameMsg" style="font-size:11px;margin-top:4px;min-height:16px;"></div>
        </div>

        <div style="margin-bottom:20px;">
          <label style="font-size:11px;font-weight:700;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:0.5px;display:block;margin-bottom:6px;">Bio <span style="font-weight:400;opacity:0.5;">(optional)</span></label>
          <textarea id="crEditBio" maxlength="150" placeholder="Tell people about yourself..."
            style="width:100%;height:70px;padding:10px 12px;border-radius:10px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.06);color:#fff;font-size:13px;outline:none;resize:none;box-sizing:border-box;font-family:inherit;">${p.bio || ''}</textarea>
        </div>

        <button id="crEditSave" style="width:100%;padding:12px;background:#ff2c2c;border:none;border-radius:10px;color:#fff;font-weight:800;font-size:14px;cursor:pointer;">Save Changes</button>
        <div id="crEditError" style="margin-top:8px;font-size:12px;color:#ff6b6b;text-align:center;min-height:16px;"></div>
      </div>
    </div>
  `;
  document.body.appendChild(modal);

  document.getElementById("crEditClose").onclick = () => modal.remove();
  modal.onclick = e => { if (e.target === modal) modal.remove(); };

  // Live avatar preview
  document.getElementById("crAvatarInput").oninput = function() {
    const preview = document.getElementById("crAvatarPreview");
    const img = document.getElementById("crAvatarImg");
    if (this.value) {
      if (img) { img.src = this.value; }
      else { preview.innerHTML = `<img id="crAvatarImg" src="${this.value}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`; }
    }
  };

  // Username availability check
  let _unameTimer;
  document.getElementById("crEditUsername").oninput = function() {
    clearTimeout(_unameTimer);
    const val = this.value.trim().toLowerCase();
    const msg = document.getElementById("crUsernameMsg");
    if (!val) { msg.textContent = ""; return; }
    if (!/^[a-z0-9_]{3,20}$/.test(val)) {
      msg.textContent = "3-20 chars, letters/numbers/underscore only";
      msg.style.color = "#ff6b6b"; return;
    }
    if (val === _crProfile?.username) { msg.textContent = "✓ Current username"; msg.style.color = "#22c55e"; return; }
    msg.textContent = "Checking..."; msg.style.color = "rgba(255,255,255,0.3)";
    _unameTimer = setTimeout(async () => {
      try {
        const { ref, get } = window._fbHelpers;
        const snap = await get(ref(_fbDb, "usernames/" + val));
        if (snap.exists()) { msg.textContent = "Username taken"; msg.style.color = "#ff6b6b"; }
        else { msg.textContent = "✓ Available"; msg.style.color = "#22c55e"; }
      } catch(e) { msg.textContent = ""; }
    }, 500);
  };

  // Save
  document.getElementById("crEditSave").onclick = async () => {
    const name     = document.getElementById("crEditName").value.trim();
    const username = document.getElementById("crEditUsername").value.trim().toLowerCase();
    const bio      = document.getElementById("crEditBio").value.trim();
    const avatarUrl = document.getElementById("crAvatarInput").value.trim();
    const errEl    = document.getElementById("crEditError");

    if (!name)     { errEl.textContent = "Display name required"; return; }
    if (!username) { errEl.textContent = "Username required"; return; }
    if (!/^[a-z0-9_]{3,20}$/.test(username)) { errEl.textContent = "Invalid username format"; return; }

    // Check username not taken (if changed)
    if (username !== _crProfile?.username) {
      try {
        const { ref, get } = window._fbHelpers;
        const snap = await get(ref(_fbDb, "usernames/" + username));
        if (snap.exists()) { errEl.textContent = "Username already taken"; return; }
      } catch(e) {}
    }

    try {
      const { ref, set, remove } = window._fbHelpers;
      const uid = _crUser.uid;

      // Remove old username mapping if changed
      if (_crProfile?.username && _crProfile.username !== username) {
        await remove(ref(_fbDb, "usernames/" + _crProfile.username));
      }

      const updated = { ..._crProfile, displayName: name, username, bio, avatarUrl, uid };
      await set(ref(_fbDb, "users/" + uid + "/profile"), updated);
      await set(ref(_fbDb, "usernames/" + username), uid);

      // Update Firebase Auth display name
      if (window._fbHelpers.updateProfile) {
        await window._fbHelpers.updateProfile(_crUser, { displayName: name, photoURL: avatarUrl });
      }
...
async function checkAndAwardAchievements() {
  ...
  try {
    ...
    const uid = _crUser.uid;
    const snap = await get(ref(_fbDb, "users/" + uid + "/achievements"));
    const earned = snap.exists() ? snap.val() : {};

    for (const ach of ACHIEVEMENTS) {
      if (earned[ach.id]) continue; // already earned
      if (ach.check(historyData, watchlistData)) {
        earned[ach.id] = { earnedAt: Date.now() };
        await set(ref(_fbDb, "users/" + uid + "/achievements"), earned);
        // Show toast
        setTimeout(() => {
...
async function followUser(targetUid) {
  ...
  try {
    const { ref, set, increment } = window._fbHelpers;
    const uid = _crUser.uid;
    await set(ref(_fbDb, "users/" + uid + "/following/" + targetUid), { followedAt: Date.now() });
    await set(ref(_fbDb, "users/" + targetUid + "/followers/" + uid), { followedAt: Date.now() });
    // Increment counts
    await set(ref(_fbDb, "users/" + uid + "/profile/following"),
      (_crProfile?.following || 0) + 1);
    await set(ref(_fbDb, "users/" + targetUid + "/profile/followers"),
      (_crProfile?.followers || 0) + 1);
...
async function unfollowUser(targetUid) {
  ...
  try {
    ...
    await set(ref(_fbDb, "users/" + uid + "/profile/following"),
      Math.max(0, (_crProfile?.following || 1) - 1));
...
async function isFollowing(targetUid) {
  ...
  try {
    const { ref, get } = window._fbHelpers;
    const snap = await get(ref(_fbDb, "users/" + _crUser.uid + "/following/" + targetUid));
    return snap.exists();
  } catch(e) { return false; }
}

// ── Session greeting (shows once per session on home page) ─────────────────
function showSessionGreeting() {
  if (!_crUser || !_crProfile) return;
  if (sessionStorage.getItem("cr_greeted")) return;
...
window.addEventListener("load", async () => {
  ...
  onAuthReady(async (user) => {
    if (user) {
      await loadFromCloud();
      showSessionGreeting();
      setTimeout(checkAndAwardAchievements, 3000);
    }
  });
});
